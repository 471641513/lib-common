	...
	for {
		...
		case msg := <-c.msgQueue:
			if tube == nil {
				...
				if err != nil {
					xlog.Error("can't connect beanstalkd server | addr: %s | tube: %s | error: %s", addr, msg.Tube, err)
			...
			if err != nil {
				xlog.Error("can't create job | addr: %s | msg: %s | error: %s", addr, msg, err)
			...
			xlog.Debug("create job | id: %d | msg: %s", id, msg)
		}
	}
}
